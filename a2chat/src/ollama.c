...
static const char SYS_PROMPT[] =
    "This chat already uses Uthernet II Ethernet to Ollama on the LAN. "
    "ProDOS paths are /VOLUME/FILE (15-char names). Never Windows C: or "
    "backslash. Short answers. Disk tools only for real ProDOS paths.";

static int wants_tools(const char *t)
{
    static const char *kw[] = {
        "catalog", "prodos", "prefix", "directory", "list_dir",
        "/cat", "/read", "volume/", ".po", 0
    };
    unsigned k, i, j;
...
static int run_tool(struct jsonscan *js, char *result, unsigned rsz)
{
    ...
    for (i = 0; path[i]; i++) {
        if (path[i] == ' ' || path[i] == '\\' || path[i] == ':') {
            strncpy(result, "ProDOS path required (/VOLUME/FILE), not C: or DOS", rsz - 1);
            return 0;
        }
    }
    ...
    if (!strcmp(js->tool_name, "read_file")) {
        ...
        int hex = 0;
        if (prodos_read(path, js->arg_offset, len, result, rsz, &got, hex) < 0) {
            return -1;
        }
        if (got >= rsz - 1) {
            strncat(result, "\n(clipped)", rsz - strlen(result) - 1);
        }
        return 0;
    }
    if (!strcmp(js->tool_name, "write_file")) {
        ...
        if (prodos_write_file(path, pay, pt, 0, 0) < 0) {
...
int ollama_send(const char *user_text, const char *attach_path)
{
    ...
    for (hop = 0; hop < MAX_HOPS; hop++) {
        ...
        if (write_body(att, hop == 0 && use_tools) < 0) {
